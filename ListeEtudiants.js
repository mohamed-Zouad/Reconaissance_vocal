...
const ListeEtudiants = () => {
  ...
  useEffect(() => {
    const fetchEtudiantData = async () => {
      try {
        ...
        console.log("error fetching etudiant data", error);
      }
    };
    fetchEtudiantData();
  }, []);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "white" }}>
      ...
      {etudiant.length > 0 ? (
        <SearchResults data={etudiant} input={input} setInput={setInput} />
      ) : (
        <View
          style={{ flex: 1, justifyContent: "center", alignItems: "center" }}
        >
          <Text>No Data</Text>
          <Text>Press on the plus button and add your Etudiant</Text>
